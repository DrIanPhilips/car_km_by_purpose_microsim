...
#This functions produces .jpg maps of the mean distance travelled per person per 
# year by LSOA for the each of the 6 trip purpose types. Hence, 6 maps are produced in 
# total. These maps are saved in the folder "Plots_Purpose_Distance/".
...
#Region refers to the Government Office Region the data is from, and is only
# used for naming the maps
plotMeanLSOA <- function(dataframe, region){
  ...
  #If you set tmap_mode('view') the map will appear in a webbrowser window and you 
  # can pan around it.  
  ...
  for (i in 1:length(tripvec)){
    ...
    #Get the ith element in of the trip purpose type.
    namei <- tripvec[i]
    ...
    #It filters to ensure there are no NA values.
    lsoa2 <- lsoa %>% 
      select((!!as.symbol(namei))) %>%
      filter((!!as.symbol(namei)) >= 0)
    ...
    #This saves the map into a folder called Plots_Purpose_Distance/
    tmap_save(m, paste0("Plots_Purpose_Distance/",datetime," In ", region," Mean Distance Travelled by Car per Person per Year by LSOA for ",namei,".jpg"),dpi = 300)
